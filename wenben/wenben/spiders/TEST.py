...
from wenben.wenben.items import DBScrapyItem
...
class douban_spyder(scrapy.Spider):
    ...
    def dbSearch(self, response):
        ...
        for div in divs:
            scrapydoubanItem['useful_count'] = div.xpath(
            ...
            scrapydoubanItem['comment_time'] = div.xpath(
        ...
        for nextpage in nextpage_urls:
            yield Request(url='https://movie.douban.com/subject/4920389/comments' + nextpage,
                          callback=self.dbSearch)  # 将下一页的URL发送到scrapy待爬队列
    ...
    def dbMoviedetail(self, response):
        ...
        with open('.\moviedetail.txt', 'w')as f:
            ...
            vote_count = selector.xpath(
                '//div[@class="rating_self clearfix"]//a[@class="rating_people"]//text()').extract()
